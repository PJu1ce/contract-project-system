...
def fetch_contract(session: Session, id: int) -> Contract | None:
    ...
    if contract is None:
        id = int(input('Введите корректный ID:'))
        return fetch_contract(session, id)
    else:
        return contract
...
def fetch_project(session: Session, id: int) -> Project | None:
    ...
    if project is None:
        id = int(input('Введите корректный ID:'))
        return fetch_project(session, id)
    else:
        return project
...
def confirm_contract(session: Session) -> None:
    '''Подтвердить договор'''
